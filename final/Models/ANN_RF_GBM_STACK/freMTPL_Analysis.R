...
rm(factors, i, factor_levels_output, factor_levels_check, reorder_factor_levels, tab)
...
if(approach_used == 1) {
  ...
  fit <- fit(mlp1, list(x_train, o_train), y = y_train, validation_split = 0.1,
  ...
  predictions_xgb_l2 <- predict(xgboost, xgb_test)
  predictions_freq <- (predictions_xgb_l2 * (max_target - min_target)) + min_target
} else {
...
if(approach_used == 1) {
  ...
  predictions_xgb_l2 <- predict(xgboost, xgb_test)
  predictions_sev <- (predictions_xgb_l2  * (max_target - min_target)) + min_target
} else {
  if(selected_model == "mlp1") {
    network <- input_data %>%
      layer_dense(units = 100, activation = "tanh", name = "h1") %>%
...
if(approach_used == 1) {
  ...
  predictions_xgb_l2 <- predict(xgboost, xgb_test)
  predictions_lc <- (predictions_xgb_l2 * (max_target - min_target)) + min_target
} else {
  if(selected_model == "mlp1") {
    set.seed(14578)
    network <- input_data %>%
...
nrmse_ML <- NRMSE(test$ML, test$ClaimAmount)
...
nrmse_glm <- NRMSE(test$GLM, test$ClaimAmount)
...
nrmse_glm_lc <- NRMSE(test$GLM_LC, test$ClaimAmount)
...
nrmse_ML_lc <- NRMSE(test$ML_LC, test$ClaimAmount)
...
nrmse_compiled <- c(nrmse_glm, nrmse_ML, nrmse_glm_lc, nrmse_ML_lc) * 100
...
model_metrics <- data.frame(models, rmse_compiled, nrmse_compiled, mae_compiled, pearson_cor_compiled, spearman_cor_compiled, gini_index_compiled, agg_rpr_compiled)
colnames(model_metrics) <- c("Model", "RMSE", "NRMSE", "MAE", "Pearson (Linear) Correlation", "Spearman Rank Correlation", "Gini Index", "Aggregate Risk Premium Differential")

model_metrics <- model_metrics %>%
  group_by(Model) %>%
  mutate(aggregate_score = mean(c(`Gini Index`, 1 - NRMSE, `Spearman Rank Correlation`, `Aggregate Risk Premium Differential`)))

View(model_metrics)
write_csv(model_metrics, "model_evaluation_stats.csv")

lift_curve(test$GLM, observed_loss_cost = test[[targetvar]], exposure = test[[exposure]], n = 20)
lift_curve(test$ML_LC, observed_loss_cost = test[[targetvar]], exposure = test[[exposure]], n = 20)

gini_index_plot_ML <- gini_plot(test$ML_LC, as.numeric(unlist(test[, c(exposure)])))
...
grouped_data <- one_way_prep(test, predictors, targetvar, test$ML_LC)

ay <- list(
  overlaying = "y", side = "right", title = "Exposure"
)
one_way_analysis_plot <- plot_ly(grouped_data, x = ~level,
                                 transforms = list(
                                   list(type = "filter", target = ~variable, operation = "=", 
                                        value = unique(grouped_data$variable)[1])
                                 ))
one_way_analysis_plot <- one_way_analysis_plot %>%
  add_lines(y = ~actuals, name = "Observed Burning Cost") %>%
  add_lines(y = ~expected, name = "Predicted Burning Cost") %>%
  add_trace(y = ~exposure, name = "Exposure", type = "bar", yaxis = "y2", opacity = 0.4) %>%
  layout(yaxis = list(title = "Burning Cost"), barmode = "group", title = "One-Way Plot of Actual vs. Expected Burning Costs", yaxis2 = ay,
         ...
         updatemenus = list(
           list(
             ...
             y = 0.6,
             buttons = list(
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[1]), label =    unique(grouped_data$variable)[1]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[2]), label = unique(grouped_data$variable)[2]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[3]), label = unique(grouped_data$variable)[3]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[4]), label = unique(grouped_data$variable)[4]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[5]), label = unique(grouped_data$variable)[5]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[6]), label = unique(grouped_data$variable)[6]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[7]), label = unique(grouped_data$variable)[7]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[8]), label = unique(grouped_data$variable)[8]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[9]), label = unique(grouped_data$variable)[9]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[10]), label = unique(grouped_data$variable)[10]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[11]), label = unique(grouped_data$variable)[11]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[12]), label = unique(grouped_data$variable)[12]), 
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[13]), label = unique(grouped_data$variable)[13]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[14]), label = unique(grouped_data$variable)[14]),
               list(method = "restyle", args = list("transforms[0].value", unique(grouped_data$variable)[15]), label = unique(grouped_data$variable)[15])
             )
           )
         )
  )

one_way_analysis_plot
...
rm(dat, dat_nn, fit, glm_freq, glm_lc, glm_sev, input_data, network, o_test, o_train, o_val, offset,
   ...
   x_val, y_test, y_train, y_val, cor_glm_lc_pearson, cor_glm_lc_spearman,
   cor_glm_pearson, cor_glm_spearman, cor_ML_lc_pearson, cor_ML_lc_spearman, cor_ML_pearson,
   cor_ML_spearman, gini_glm, gini_glm_lc, gini_index_compiled, gini_ML, gini_ML_lc,
   ...
   w_test, w_train, w_val, weights, xgb_test, xgb_train, extract_levels, 
   loss_tweedie, tweedie_deviance_loss, exposure, predictions_l2_lc, agg_rpr_glm_lc,
...
flash <- add_shap(flash, visit_strategy = "importance", n_shap = 50)
...
imp_combined <- imp_combined %>%
  group_by(Variable) %>%
  mutate(composite_variable_importance_score = mean(c(scaled_shap_importance, scaled_dropout_loss)))
...
#ALE Profiles
ale <- FeatureEffects$new(predictor, predictors, method = "ale", center.at = 0)
...
pd_profile <- list()
for(i in 1:length(predictors)) {
  pd <- light_profile(flash, v = predictors[i], type = "partial dependence", pd_n_max = 0.75 * nrow(test))$data
  pd <- pd %>%
    select(-c(label, type)) %>%
...
for(m in 1:nreps) {
  groups <- createFolds(test$Record_ID, nfolds)
