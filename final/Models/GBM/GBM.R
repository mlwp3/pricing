...
eval_dataset %$% NRMSE(predicted_loss_cost, observed_loss_cost)

eval_dataset %$% gini_plot(observed_loss_cost, predicted_loss_cost_freq_sev, Exposure) + 
  ggtitle("Gini index Freq / Sev") + ggsave("./Output/GBM/gini_freq_sev.png")

eval_dataset %$% gini_plot(observed_loss_cost, predicted_loss_cost, Exposure) + 
  ggtitle("Gini index Loss Cost") + ggsave("./Output/GBM/gini_loss_cost.png")

eval_dataset %$% gini_value(observed_loss_cost, predicted_loss_cost_freq_sev, Exposure)

eval_dataset %$% gini_value(observed_loss_cost, predicted_loss_cost, Exposure)
...
eval_dataset %$% lift_curve_table(predicted_loss_cost, observed_loss_cost, Exposure, 20) %>% 
                 lift_curve_plot() +
                 ggtitle("Lift Curve Loss Cost") + ggsave("./Output/GBM/lift_curve_loss_cost.png")

eval_dataset %$% double_lift_chart(predicted_loss_cost_freq_sev, predicted_loss_cost, observed_loss_cost, Exposure, 20, "Freq / Sev", "Loss Cost") + 
...
test %>% select(RecordID,
                ...
                select(RecordID,
                       ...
                       predicted_loss_cost_freq_sev,
                       predicted_loss_cost) %>%
                write_csv("./Output/GBM/dataset_predictions_gbm.csv")
