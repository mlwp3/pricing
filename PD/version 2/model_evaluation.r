...
source("./perf_eval.R")
#import data
test_output <- read_csv("LC Test Predictions Output.csv")
...
write_csv(model_metrics, "../../Output/RF/LC_RF_model_evaluation_stats.csv")

# write charts

test_output %$% gini_plot(ClaimAmount_predicted, Exposure) + ggtitle("Gini index Loss Cost") + ggsave("../../Output/RF/LC_gini.png",  scale = 3, dpi = 300)

test_output %$% lift_curve_table(ClaimAmount_predicted, ClaimAmount, Exposure, 20) %>% 
  lift_curve_plot() + ggtitle("Lift Curve Freq / Sev") + ggsave("../../Output/RF/LC_lift_curve.png",  scale = 3, dpi = 300)

test_output %$% lift_curve_table(ClaimAmount_predicted, ClaimAmount, Exposure, 20) %>% 
  lift_curve_plot() + ggtitle("Lift Curve Loss Cost") + ggsave("../../Output/RF/LC_lift_curve.png",  scale = 3, dpi = 300)
